...
=== 参数总表 ===
  ...
  t_ref_abs   : 绝对不应期 (1-2 ms, 发放后完全不可激发)
  t_ref_rel   : 相对不应期 (3-5 ms, 需要更强刺激才能再次发放)
...
>
> **实验证据说明:**
> - Ca²⁺ 树突脉冲在 **L5 锥体细胞**中有强有力的实验证据 (Larkum et al. 1999, 2004)
> - 在 **L2/3 锥体细胞**中，Ca²⁺ 树突脉冲也有实验支持 (Larkum et al. 2007, JNeurosci)，
>   但其顶端树突较短、机制与 L5 不完全相同，burst 能力较弱
> - 因此 L2/3 的 κ=0.3 (中等耦合) 是对此差异的合理建模近似
> - L5 的 κ=0.6 (强耦合) 反映其最长顶端树突和最强 burst 能力
>
> **反向传播动作电位 (bAP) 的建模:**
> 双区室模型中，bAP 效应隐含在胞体→顶端的耦合项 `κ · (V_s - V_a)` 中。
> 当胞体发放时 V_s 瞬间升高，通过耦合使 V_a 去极化，模拟了 bAP 到达顶端树突的效应。
> 这种 bAP + 顶端输入的配合是触发 Ca²⁺ 树突脉冲的关键机制 (Larkum et al. 1999)。
...
NMDA 突触电流 (电压依赖性 Mg²⁺ 阻断):
  ...
  参数: τ_NMDA = 100 ms (范围 40-150 ms), τ_rise = 5 ms (范围 3-15 ms)

GABA_A 突触电流:
  I_GABA_A = g_GABA_A · s_GABA · (V - E_inh)
  参数: τ_GABA_A = 6 ms, E_inh = -70 mV (范围 -70 ~ -75 mV, 取决于 Cl⁻ 浓度)
...
│
├── ShortTermPlasticity (短时程可塑性, Tsodyks-Markram 模型)
│   ├── STD                    # 短时程抑制 (突触囊泡耗竭, τ_D ~200-800 ms)
│   └── STF                    # 短时程促进 (残余 Ca²⁺ 积累, τ_F ~50-200 ms)
│   # 参数: U (初始释放概率 0.1-0.9), τ_D (恢复时间), τ_F (促进时间)
│   # 丘脑→皮层突触: 强STD (高U~0.5-0.9, 快速适应)
│   # 皮层→皮层突触: STF为主 (低U~0.1-0.3, 频率增强)
│   # 抑制性突触: 类型依赖 (PV+: STD; SST+: STF)
│
...
```
Tsodyks-Markram 模型 (突触动态):

  突触释放概率:
    u(t) → u(t) + U · (1 - u(t))         当突触前脉冲到达时
    du/dt = -u / τ_F                      脉冲间衰减 (促进变量)

  可用囊泡比例:
    x(t) → x(t) - u(t) · x(t)            当突触前脉冲到达时 (释放消耗)
    dx/dt = (1 - x) / τ_D                 脉冲间恢复 (抑制变量)

  有效突触权重:
    w_eff = w · u · x                     实际传递强度 = 静态权重 × 释放概率 × 可用资源

  参数:
    U     : 初始释放概率 (0.1-0.9, 突触类型依赖)
    τ_F   : 促进时间常数 (50-200 ms)
    τ_D   : 抑制/恢复时间常数 (200-800 ms)

  不同突触类型的 STP 特征:
    丘脑→L4 (前馈):    U=0.5-0.9, τ_D=200ms, τ_F=20ms   → 强STD (快速适应, 突出瞬态/新奇)
    L2/3→L2/3 (侧向):  U=0.1-0.3, τ_D=800ms, τ_F=200ms  → 强STF (频率增强, 持续整合)
    L5→皮层下 (输出):   U=0.3-0.5, τ_D=300ms, τ_F=100ms  → 混合型 (兼顾响应性和稳定性)
    PV+→锥体 (抑制):   U=0.3,     τ_D=200ms, τ_F=20ms   → STD (高频时抑制减弱)
    SST+→锥体 (抑制):  U=0.1,     τ_D=800ms, τ_F=200ms  → STF (持续活动时抑制增强)
```
...
[MultiColumn System]  ← 依赖 Router + Modulator
    ...
    └──→ [AmygdalaValence]      ← 依赖 MultiColumn
              │
              ▼
    [PFC CorticalColumns 整合]  ← PFC 只是参数不同的 CorticalColumn
              │                    (不再有独立 PrefrontalExecutive 模块)
              ▼
    [悟韵 (WuYun) 全系统整合]
```
...
```
睡眠周期 (~90 分钟/周期, 每夜 4-6 个周期):

┌─────────────────────────────────────────────────────────┐
│  NREM 阶段 (占 ~75-80%)                                │
│                                                         │
│  N1 (入睡): α→θ 过渡                                   │
│  N2 (浅睡): 纺锤波 (12-15 Hz, ~0.5-2s) + K-complex    │
│  N3 (深睡/慢波睡眠 SWS):                                │
│    - 慢振荡 (SO, 0.5-1 Hz): 皮层 UP/DOWN 状态交替     │
│    - 锐波涟漪 (SWR, ~150-250 Hz): 海马 CA3/CA1 重放    │
│    - 纺锤波: 丘脑产生, 嵌套在 SO 的 UP 状态中           │
│    - 时序: SO UP 状态 → 触发 SWR → 嵌套纺锤波           │
│    - 功能: 海马→皮层记忆巩固 (陈述性记忆)               │
│                                                         │
│  REM 阶段 (占 ~20-25%)                                  │
│    - θ 振荡 (4-8 Hz): 海马活跃                          │
...
1. NREM 慢波阶段:
   - 皮层 SO 模拟: 所有皮层柱交替进入 UP/DOWN 状态
     DOWN: 所有神经元沉默 (V_s → V_rest, 无外部输入)
     UP:   自发活动 (内部循环连接驱动)
   - SWR 触发: 海马 CA3 自联想网络自发重激活存储的模式
     频率: 每 50-100 ms 一次 ripple event
     重放速度: ~10-20x 加速 (相对于编码时的时间)
   - 纺锤波: 丘脑 TRN-TC 循环产生 12-15 Hz 振荡
     功能: 为皮层 STDP 提供时间窗口 (纺锤波期间可塑性增强)
   - 巩固: SWR 重放 → 通过 SC→皮层 通路 → 驱动皮层 STDP
     重复重放 → 皮层突触逐渐强化 → 记忆从海马迁移到皮层

2. REM 阶段:
   - 海马 θ 振荡恢复
   - 皮层接收内源性随机输入 (模拟 PGO 波)
   - ACh 水平升高 → 增强可塑性 (但方向与清醒不同)
   - 功能: 跨模态记忆整合、弱关联发现

3. 巩固时钟 (仿真实现):
   仿真时间达到 consolidation_interval 时触发 mini-sleep:
   - 暂停外部输入
   - 执行 N 轮 SWR 重放 (N 由近期学习量决定)
   - 应用加速的皮层 STDP
   - 可选: 突触缩放 (稳态可塑性, 防止权重爆炸)
   - 恢复外部输入
```
...
人脑能量预算:
  - 总功耗: ~20 W (占身体能耗 ~20%)
  - 皮层每个神经元: ~10⁹ ATP/s
  - 脉冲发放成本: ~10⁸ ATP/spike (是静息代谢的 ~10 倍)
  → 核心约束: 高发放率 = 高能耗 → 进化压力偏好稀疏激活

稀疏性来源:
  1. 低平均发放率: 皮层 ~1-5 Hz (不是每个时间步都发放)
  2. 群体稀疏性: 同一时刻只有 ~1-5% 的神经元活跃
  3. 突触稀疏性: 每个神经元只连接到 ~0.1% 的其他神经元
  4. 预测编码效率: 预测准确时只有沉默/burst, 不产生向上误差
```
...
能量追踪 (可选模块):
  ...
  每个时间步的能量消耗:
    E_total = Σ E_spike(i) + Σ E_synapse(j) + E_baseline

  其中:
    E_spike(i) = E_AP · spike(i)           # 每个脉冲的 ATP 成本
    E_synapse(j) = E_syn · |I_syn(j)|      # 每个活跃突触的传递成本
    E_baseline = N_neurons · E_rest         # 静息代谢

  稀疏性约束机制:
    1. 适应变量 w: 高频发放 → w 增大 → 阈值升高 → 自动降频
    2. 稳态可塑性: 保持每个神经元的平均发放率在目标范围内
    3. 抑制性网络 (E/I 平衡): PV+ 篮状细胞维持竞争稀疏
    4. 预测编码: 准确预测 → 误差减少 → 脉冲减少 → 节能
...
```
悟韵 (WuYun) 发育阶段 (映射到仿真时间):

阶段 1: 初始化 (对应胎儿期/新生儿)
  - 建立解剖连接拓扑 (由 configs/connectome/ 定义)
  - 突触权重随机初始化 (在生物合理范围内)
  - 神经元参数设为初始值
  - 此时系统行为类似新生儿: 基本反射、无识别能力

阶段 2: 关键期 (对应婴幼儿期)
  - 可塑性窗口打开: 特定脑区的 STDP 学习率提升 2-5 倍
  - 感觉皮层关键期: V1/A1 的可塑性最强, 快速形成特征选择性
  - 连接修剪: 使用率低的突触被削弱/删除 (结构可塑性)
  - 关键期顺序: 感觉皮层 → 联合皮层 → PFC (与人脑一致)

阶段 3: 青少年期
  - 感觉皮层关键期逐渐关闭 (可塑性降回基线)
  - PFC 关键期仍然开放 (执行功能仍在发育)
  - 突触修剪继续 (去除冗余连接, 提高效率)

阶段 4: 成年期
  - 大部分关键期关闭
  - 保留在线学习能力 (但速度更慢)
  - 海马仍可快速编码新记忆 (one-shot)
  - 成年神经发生: DG 保持少量新颗粒细胞生成
```
...
关键期开关 (参数调制):
  ...
  critical_period_state = {
      region_id: {
          'is_open':        bool,     # 当前是否在关键期
          'plasticity_gain': float,   # STDP 学习率乘数 (关键期: 2-5x)
          'pruning_rate':    float,   # 结构可塑性修剪速率
          'open_time':       int,     # 关键期开始时间 (仿真步)
          'close_time':      int,     # 关键期结束时间 (仿真步)
      }
  }

  关键期调控因子:
    1. PV+ 抑制成熟: PV+ 篮状细胞的抑制强度逐渐增加
       → 达到阈值时关键期关闭 (Hensch 2005)
    2. ACh 水平: 高 ACh → 增强可塑性 → 延长关键期
    3. BDNF (简化为参数): 促进 PV+ 成熟 → 加速关键期关闭
    4. 髓鞘化 (简化): 连接延迟逐渐降低 → 时序精度提高

  ⚠️ 反作弊约束:
    关键期只调节"学习速率"和"可塑性窗口"
    不能在关键期中注入预设的权重或功能
    关键期结束后仍保留学习能力, 只是速率降低
```
