...
│       ├── bcm.py                 # BCM 理论
│       ├── stp.py                 # 短时程可塑性 (Tsodyks-Markram: STD/STF)
...
│   ├── sleep_system.py            # ★ 睡眠/离线处理系统 (NREM慢波-纺锤波耦合, REM重激活, 海马→皮层重放)
│   ├── sensory_interface/         # ★ 感觉输入编码接口 (外部信号→脉冲)
│   │   ├── __init__.py
│   │   ├── visual_encoder.py      # 视网膜→LGN 编码 (边缘/对比度/运动提取)
│   │   ├── auditory_encoder.py    # 耳蜗→MGN 编码 (时频变换/频率分解)
│   │   ├── somatosensory_encoder.py  # 触觉→VPL 编码
│   │   └── text_encoder.py        # 文本→语言区输入编码 (实验用)
│   └── motor_interface/           # ★ 运动输出解码接口 (脉冲→外部动作)
│       ├── __init__.py
│       ├── motor_decoder.py       # M1 脉冲→动作指令解码
│       ├── speech_decoder.py      # Broca→语音输出解码
│       └── text_decoder.py        # 语言区→文本输出解码 (实验用)
│
│  ╔═══════════════════════════════════════════════════════╗
│  ║  核心引擎                                             ║
│  ╚═══════════════════════════════════════════════════════╝
...
│   │   └── ...
│   ├── thalamus/
│   │   └── thalamic_nuclei.yaml   # 丘脑核团参数 (初始连接概率、TRN抑制参数)
│   └── neuromodulator/
...
│       ├── stdp.yaml              # A+, A-, τ+, τ- 等
│       ├── da_stdp.yaml           # 三因子参数
│       └── stp.yaml               # 短时程可塑性 (U, τ_D, τ_F 按突触类型)
│
├── connectome/                    # 连接矩阵配置
...
│
├── schema/                        # 配置schema说明
│   └── config_schema.yaml         # 配置文件结构说明
│
└── sleep/                         # 睡眠/巩固参数
    ├── nrem_params.yaml           # NREM 慢波-纺锤波耦合参数
    ├── rem_params.yaml            # REM 重激活参数
    └── consolidation.yaml         # 记忆巩固调度参数 (SWR频率、重放次数等)
```
...
| (待创建) 睡眠与巩固机制设计 | `system/sleep_system.py` + `configs/sleep/` | 离线处理与记忆巩固 |
| (待创建) 感觉/运动接口设计 | `system/sensory_interface/` + `system/motor_interface/` | 外部世界↔脉冲编码转换 |
| (待创建) 发育与关键期机制 | `engine/` + `configs/` | 连接修剪时间线、关键期开关、成年神经发生 |
