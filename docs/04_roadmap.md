...
> 基于 6 篇 2024-2025 文献审查后的高优先级修补

- ✅ **3.1-A**: NMDA 电压门控 — `wuyun/synapse/synapse_base.py`
  - Mg²⁺ 阻断因子: B(V) = 1/(1 + [Mg²⁺]/3.57 · exp(-0.062·V))
  - 静息态(-65mV) B≈0.1 几乎完全阻断，去极化(-35mV) B≈0.5 开始传导
  - NMDA 成为真正的重合检测器 (Jahr & Stevens, 1990)
  - 预测编码硬件基础: 只有前馈+反馈同时存在才增强突触传导
- ✅ **3.1-B**: L6→TC 反馈增强 — `wuyun/circuit/multi_column.py`
  - column_to_tc_gain: 10→25 (预测信号驱动 TC apical burst)
  - column_to_trn_gain: 5→15 (负反馈抑制非预测信号)
  - 生物学依据: 皮层→丘脑反馈是前馈投射的 ~10 倍
- 📊 **累计 56/56 测试通过** (零回归, 2026-02-07)
